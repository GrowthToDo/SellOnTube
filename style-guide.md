...
This document defines the writing style rules for all SellonTube blog content. Every draft must follow these rules. When in doubt, read the sentence out loud — if it sounds like something no one would say in a real conversation, rewrite it.

---

## Fix #1: No Em-Dashes

Never use em-dashes (—). Rewrite any sentence that needs one. Use periods, commas, or colons instead. If a sentence needs an em-dash to work, it's probably trying to do too much. Break it into two sentences.

---

## Fix #2: No AI Transition Patterns

Ban these paragraph openers entirely:
...
These are allowed once per post maximum (never twice):
...
- **"Not only X, but also Y."** Overused by AI. Real writers rarely use it.
- **"Whether you're X or Y, this Z..."** AI filler. Just start with the point.
- **Triple-adjective stacks.** "A comprehensive, actionable, and practical guide." One adjective is enough. Two max.
- **"Let's dive in" / "Without further ado."** Just start the next section.
...
Never write paragraphs that are all the same length. Real writing has texture.

**Rules:**

- Mix paragraph lengths deliberately. Follow a 3-sentence paragraph with a 1-sentence paragraph. Then a 2-sentence one.
- Use 2-3 one-sentence paragraphs per post for emphasis.
- Never have more than 4 consecutive paragraphs of the same length.
- Vary sentence openings. If three consecutive sentences start with "You" or "This" or "The," rewrite two of them.

**Bad (metronome rhythm):**

"YouTube marketing is a powerful channel for small businesses. It allows you to reach customers who are actively searching for solutions. This means you can target high-intent buyers.

Another benefit is that videos compound over time. Unlike paid ads, a video published today can still bring traffic next year. This makes YouTube a long-term investment."

**Better:**

"YouTube marketing works because people search YouTube like they search Google — with intent to solve a problem.

That's different from social media, where you're interrupting someone's scroll. On YouTube, they came looking for you. And a video you publish today can still bring leads 18 months from now."

---

## Fix #5: Write Better Headings

Ban these heading patterns:

- "Understanding X"
- "The Importance of X"
- "Key Considerations for X"
- "Exploring X" / "Navigating X"
- "A Comprehensive Guide to X"

**The rule:** Every H2 and H3 should state a clear benefit, ask a question the reader actually has, or make a specific claim.

| Bad | Better |
|-----|--------|
| Understanding YouTube SEO for Shopify | Why Your Shopify Store's YouTube Channel Isn't Driving Sales |
...
**Never use these as headings:** "Conclusion," "Final Thoughts," "Wrapping Up," "In Summary."

**Never do these:**

- Restate the main points in bullet form at the end
- End with "By following these steps, you'll be well on your way to..."
- Close with vague motivation: "The time to start is now" or "The only question is: are you ready?"
...
- "For example, let's say you run a Shopify store..." (hypothetical disguised as example)
- "Imagine you're a coach who..." (fictional scenario, zero credibility)
- "Companies like [big brand] have seen success with..." (namedropping without data)
- "This could lead to increased conversions" ("could" + "increased" = said nothing)

**The rule:** Every example must have at least 2 of these 3: a specific number, a specific type of business, a specific outcome.
...
## Fix #8: Control List and Bold Overuse

**Bold text rules:**

- Maximum 3-4 bolded phrases per 1,000 words
- Only bold something if a scanning reader needs to see it
- Never bold entire sentences. Bold a key phrase within a sentence
- Never bold the first sentence of every paragraph (classic AI pattern)

**Bullet point rules:**

- Maximum 2 bulleted lists per 1,000 words
- Never use bullets for something that reads fine as a sentence
- Never stack more than 5 bullets in a row
- Bullet points should be at least 1-2 sentences each, not fragments

**The test:** Zoom out on the post and squint. If it looks like a PowerPoint slide covered in bullets and bold patches, it's overformatted. A good blog post looks like mostly paragraphs with occasional structure.
...
- If a sentence sounds like a university essay, rewrite it. "It is imperative to consider the ramifications of..." never belongs here.
...
The first sentence is the most important line of the post. Never waste it on a generic setup.
...
- "As a business owner, you're always looking for..."
- "You've probably heard that [topic] is important."
- Any opening that is *about* the topic rather than *inside* the topic

**The rule:** The first sentence drops the reader into a problem, a tension, or a specific scenario. No warm-up. No throat-clearing.
...
**Bad:** "When it comes to Shopify email marketing, getting the strategy right is crucial for success."
...
- **Format:** SVG — self-contained, no external assets, no photos. Astro serves SVGs directly without conversion.
- **Canvas:** 1200 × 675px — true 16:9. Correct for Google search previews, LinkedIn, Twitter/X, and WhatsApp link previews.
- **Aspect ratio:** Always 16:9 — `viewBox="0 0 1200 675" width="1200" height="675"`. Never crop or stretch.

### Visual style

- **Pure text-based graphic** — no photos, no illustrations, no external image references. The design uses only SVG shapes, gradients, and text.
- **Consistency:** All feature images are built from the same template. A reader scanning the blog index sees a coherent visual identity. Swap the headline text and category label only — everything else stays locked.
- **No numbers in headlines** — neither arabic numerals nor spelled-out numbers in the title lines. "Four Videos a Month" becomes "Videos a Month Compound." Numbers are distracting at thumbnail size.

### Composition

- **Centered layout:** All text is `text-anchor="middle"` at `x="600"`. No split column, no clipPath.
- **Title: 2 lines at 90px.** Line 1 is white (`#ffffff`), line 2 uses the brand gradient (`#3b82f6` → `#8b5cf6`). Both lines are 90px/800 weight, letter-spacing -2.
- **Vertical positions (fixed — do not change):**
  - Category pill: y=125, h=28
  - Title line 1: y=283
  - Title line 2: y=388
  - Divider bar: y=408
  - Subtitle: y=450
  - Footer wordmark: y=645
- **Background:** diagonal gradient `#030620` → `#0a1540` plus a radial glow at top-right (`#3b82f6`, 15% opacity).
- **Dot grid:** 9 cols × 5 rows of 1.5px circles at opacity 0.05 — adds texture, never distracts.
- **No small text:** Any text smaller than 20px is invisible at thumbnail size. Subtitle and branding are for full-size viewing only.

### Branding

- **Footer wordmark:** centered at x=600, y=645. "SellOnTube" in 14px/700 `rgba(255,255,255,0.88)` + " — YouTube Acquisition for B2B" in 14px/400 `rgba(229,236,246,0.32)`. Wordmark only — no URL.
- **Colour palette:**
  - Dark background: `#030620` → `#0a1540` (gradient)
  - Category pill fill: `#0161EF` at 18% opacity, stroke `#3b82f6` at 45% opacity
  - Category label text: `#60a5fa`
  - Gradient text: `#3b82f6` → `#8b5cf6` (userSpaceOnUse, x1=300 x2=900)
  - White headline: `#ffffff`
  - Subtitle: `rgba(229,236,246,0.55)`
- **Typography:** `'Inter', ui-sans-serif, system-ui, sans-serif` — the site's actual font stack.

### SEO

- **Filename:** `[post-slug]-featured.svg` — e.g. `youtube-marketing-b2b-featured.svg`.
- **Alt text:** Describe what the image shows plus the post topic. Max 125 characters. Not just the post title repeated verbatim.
- **No keyword stuffing:** One natural description only.

### File organisation

- **Storage path:** `src/assets/images/blog/[post-slug]-featured.svg`
- **One feature image per post** — no variants or A/B versions stored in the repo
- **Frontmatter:** Always reference via `~/assets/images/blog/[post-slug]-featured.svg` in the post `image` field

### Approved template spec

**Reference implementation:** `src/assets/images/blog/create-youtube-channel-for-business-featured.svg` — copy this file and swap the text content only.

**Canvas:** `viewBox="0 0 1200 675" width="1200" height="675"`

**Gradients (in `<defs>`):**
- `bg`: linearGradient, `#030620` (0%) → `#0a1540` (100%), x1=0% y1=0% x2=100% y2=100%
- `gradText`: linearGradient, `#3b82f6` (0%) → `#8b5cf6` (100%), x1=300 x2=900, gradientUnits=userSpaceOnUse
- `glow`: radialGradient, cx=75% cy=25% r=55%, `#3b82f6` 15% opacity → `#030620` 0% opacity

**Layers (top to bottom):**
1. Background rect: `fill="url(#bg)"`
2. Glow rect: `fill="url(#glow)"`
3. Dot grid: 45 circles (9 cols × 5 rows), cx values 110–1070 (step 120), cy values 110–590 (step 120), r=1.5, opacity=0.05, fill=`#ffffff`
4. Category pill (two overlapping rects, same x/y/w/h/rx): fill rect + stroke rect
5. Category label text
6. Title line 1 (white)
7. Title line 2 (gradient)
8. Divider: `x=556 y=408 width=88 height=3 rx=1.5`, `#3b82f6` 65% opacity
9. Subtitle text
10. Footer wordmark text

**Category pill:**
- Both rects: `rx=14`, centred on x=600 (adjust x and width to fit label text)
- Fill rect: `fill="#0161EF" fill-opacity="0.18"`
- Stroke rect: `fill="none" stroke="#3b82f6" stroke-width="1" stroke-opacity="0.45"`
- Label: `font-size="12" font-weight="600" letter-spacing="2.5" fill="#60a5fa" text-anchor="middle"` — UPPERCASE

**Title lines:**
- Both: `x="600" font-family="'Inter', ui-sans-serif, system-ui, sans-serif" font-size="90" font-weight="800" letter-spacing="-2" text-anchor="middle"`
- Line 1 (`y="283"`): `fill="#ffffff"`
- Line 2 (`y="388"`): `fill="url(#gradText)"`

**Footer:** `x="600" y="645" font-size="14" text-anchor="middle"` — two `<tspan>` elements

### Common mistakes — do not repeat

1. **Never use numbers in title lines** — arabic or spelled-out. They clutter the design at thumbnail size. Rephrase to remove them.

2. **Never define the same SVG attribute twice in one element** — triggers "Attribute redefined" and breaks rendering.

3. **`&` in SVG text must be escaped as `&amp;`** — unescaped ampersands cause an XML parse error and the file renders blank.

4. **gradText gradient uses userSpaceOnUse** — `gradientUnits="userSpaceOnUse"` with fixed pixel coordinates (x1=300 x2=900). Without this, the gradient maps to the bounding box of the text element and renders as a flat colour.
