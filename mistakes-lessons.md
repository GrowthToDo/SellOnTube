...
### 2026-07-21 — A scheduler trusted its own HTTP 200 for 10 days while every post silently died
- **What happened:** LinkedIn posting had been dead since 2026-07-20 and nobody noticed. `linkedin-schedule.js` POSTed 10 posts to Zernio on Jul 20, got HTTP 200 for each, wrote `status: "scheduled"` into `linkedin-history.json`, and printed `SCHEDULED` ten times. At 13:30 UTC the first post attempted to publish, failed, and Zernio flipped the LinkedIn account to `isActive: false` at 13:31:15. The Jul 21 post was auto-cancelled with `Account was disconnected`; the remaining eight sat `pending` against a dead account. X kept working the whole time because it ships through upload-post, an entirely separate vendor, so the healthy pipe masked the dead one.
- **Root cause:** two structural gaps. (1) Nothing ever compared local intent against vendor server state — scheduling success and publishing success are different events, and only the first was ever checked. (2) Two vendors wrote to two platforms with no shared health signal, so one dying produced no visible symptom anywhere.
- **Lesson:** an API 200 at schedule time is a receipt for *accepting* work, not evidence the work will happen. Anything scheduled for the future must be reconciled against the provider's own record afterwards, or a vendor-side state change is invisible until someone happens to look at the live account. This is the second time this exact shape has bitten (see 2026-07-18 `firstComment`): the provider returns success and quietly does nothing.
- **Prevention rule:** every scheduled-publish integration ships with a reconciliation check that reads the provider's state back and exits non-zero on any discrepancy (`npm run x:verify` / `scripts/x-agent/verify-x-scheduled.js` is the reference implementation). Run it after every scheduling run. Reconcile against the *history* of what was actually sent, not the queue of what was intended, or the check goes permanently red once a batch rotates and stops being read. Never treat a scheduler's own log as evidence of delivery.
- **Category:** process

### 2026-07-21 — An unpaginated delete sweep hit the right rows by luck of sort order
- **What happened:** To clear 10 dead scheduled posts from Zernio, `GET /api/v1/posts` was called with no pagination and the returned 10 posts were deleted. It worked. Only afterwards did a follow-up read reveal the endpoint defaults to 10 per page and the account actually held 19 posts, 17 of them `published`. The delete had touched exactly the right rows solely because the API happens to sort `scheduledFor` descending, which put all the future posts on page 1.
- **Root cause:** treating a single unpaginated response as the complete result set, on a destructive operation, against an API whose page size had never been checked.
- **Lesson:** "it produced the right answer" and "the method was sound" are different claims. A destructive operation that succeeds by coincidence will fail the moment the coincidence does — a different sort order, one more scheduled post, a changed default page size.
- **Prevention rule:** any list-then-mutate loop paginates until a short page is returned, and prints the full manifest before mutating. `listPosts()` in `scripts/lib/zernio.js` does this correctly; use it rather than hand-rolling a fetch. For destructive calls specifically, verify the total count against the API's own `pagination.total` before acting.
- **Category:** data

### 2026-07-21 — Deleted a "duplicate" without verifying the thing it duplicated existed
- **What happened:** A twitter post sat in Zernio scheduled for Jul 30, byte-identical to the Jul 30 entry in `queue.json`. It was diagnosed as a double-post risk against upload-post and deleted. It was not a duplicate. upload-post had no Jul 30 post: the original run's X call for that date had failed and consumed a quota unit without creating anything, and someone had patched the gap by hand in Zernio. Deleting the patch removed the only Jul 30 X post that existed. It could not be restored because upload-post's monthly quota was exhausted.
- **Root cause:** the word "identical" was doing unearned work. Content matching `queue.json` is equally consistent with "a duplicate" and "a manual patch copied from the queue". The distinguishing evidence — whether upload-post actually held a Jul 30 post — was never checked, and the endpoint that would have shown it (`/api/uploadposts/schedule`) was found only afterwards.
- **Lesson:** a deletion justified by "this duplicates X" is only as good as the proof that X exists. Matching content proves common ancestry, not redundancy.
- **Prevention rule:** before deleting anything as redundant, positively confirm the surviving copy exists by reading it from its own system. State the check explicitly ("confirmed upload-post holds Jul 30, id …") rather than inferring it. If the confirming endpoint cannot be found, that is a reason to stop, not to proceed.
- **Category:** process

### 2026-07-21 — Parallel drafting agents cannot see each other, so they wrote the same story twice
- **What happened:** Five subagents drafted 4 tweets each from different blog posts, in parallel, each blind to the others. Every group passed its own validation. The merged batch contained two collisions: two tweets told the same 12-month YouTube-vs-blog study with the same figures (2.5x → ~5x), and two used a CRM "15 person agency" example scheduled one day apart. The mechanical validator could not see either, because it only dedups on the first line and both pairs opened differently.
- **Root cause:** fan-out parallelism removes exactly the shared context that prevents repetition. Per-agent validation is necessarily local; collision is a property of the whole set.
- **Lesson:** anything fanned out to parallel agents needs a central pass that inspects the combined output for properties no individual agent could evaluate. Independent correctness does not compose into collective coherence.
- **Prevention rule:** after any parallel content fan-out, run a cross-item collision scan before presenting or shipping: shared distinctive figures, shared worked examples, and repeated premises across the whole set. A numeric-overlap scan (no figure appearing in two items) catches most of it cheaply; a human read catches the rest. Give agents a list of topics already covered by their siblings when the assignment is known up front.
- **Category:** content
