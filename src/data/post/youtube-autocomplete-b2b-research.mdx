...
**The alphabet soup technique for YouTube autocomplete is a keyword research method where you type a seed keyword, then append each letter of the alphabet (a through z) one at a time to surface the hidden search suggestions behind that seed.** Each letter triggers a different set of real, user-generated autocomplete predictions, turning one seed keyword into 260+ topic variations pulled straight from YouTube-native search demand.
...
The alphabet soup technique generates volume. Filtering generates quality. A raw autocomplete dump of 200 keywords will include everything from beginner tutorials to entertainment queries. Many of those will never produce a lead for your business.
...
Here is how those four modifier types map to search intent and an example query you can copy:

| Modifier type | Search intent | Example query |
| --- | --- | --- |
| Comparison ("vs", "comparison", "alternative to") | Active evaluation | HubSpot vs Salesforce CRM |
| Solution-seeking ("how to choose", "best [product] for", "how to set up") | A problem the searcher will pay to fix | best CRM for law firms |
| ROI and cost ("pricing", "ROI", "worth it") | Budget-stage research | is HubSpot worth it for small business |
| Industry-specific ("for agencies", "for enterprise", "for [role]") | Narrow, qualified audience | for marketing directors |
...
Prefer to skip the manual work? [our video keyword finder tool](/tools/youtube-video-keyword-finder) runs these checks automatically.
...
Seven seed keywords, each generating 15 to 30 autocomplete suggestions. That is 100 to 200 potential video topics from a single afternoon of research, which is exactly [how SaaS companies use YouTube for customer acquisition](/youtube-for/saas) at scale.
...
2. Run the alphabet soup technique on each one, or [use our YouTube autocomplete keyword tool](/tools/youtube-autocomplete-keywords) to pull all suggestions instantly. Collect results in a spreadsheet.
